...
public class GetTemplatesByDocumentType
{
    ...
    [Function("GetTemplatesByDocumentType")]
    public async Task<HttpResponseData> Run(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get")] HttpRequestData req)
    {
        var documentTypeId = System.Web.HttpUtility.ParseQueryString(req.Url.Query)
            .Get("documentTypeId");
        ...
        var results = service.RetrieveMultiple(query).Entities
            .Where(e =>
                e.GetAttributeValue<EntityReference>("ilx_documenttype")?.Id == docTypeGuid
            )
            .Select(e => new
            {
                id = e.Id,
